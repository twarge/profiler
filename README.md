...
A native macOS laser beam profiler that uses a Sony α7C — or any UVC camera — as the
sensor. Profiler is a single-window SwiftUI app: connect a camera and the window shows the
live beam with integrated horizontal and vertical profiles, ISO 11146 second-moment widths,
centroid, ellipticity and azimuth, and a closed-loop ISO servo.
...
`make` runs exactly what Xcode's own Build does — same scheme, same DerivedData — so a
`make` build and a ⌘B are the same build.
